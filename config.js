...
function createBusinessConfig(type){const modules={dashboard:true,sales:true,products:true,inventory:true,customers:true,suppliers:true,purchases:true,cash:true,reports:true};Object.keys(SPECIAL_MODULES[type]||{}).forEach(k=>modules[k]=true);return modules}
function getBusinessType(id){return BUSINESS_TYPES.find(x=>x.id===id)}
